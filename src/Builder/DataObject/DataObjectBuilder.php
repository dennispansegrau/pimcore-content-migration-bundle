...
class DataObjectBuilder extends AbstractElementBuilder
{
     ...
     * @throws Exception
     */
    public function setObjectMetadata(string $property, mixed $objectMetadataData): static
    {
        $setter = 'set' . ucfirst($property);

        [$fieldName, $object, $columns, $data] = $objectMetadataData;
        $objectMetadata = new DataObject\Data\ObjectMetadata($fieldName, $columns, $object);
        $objectMetadata->setData($data);
     ...
     * @throws Exception
     */
    public function setObjectMetadataList(string $property, array $list): static
    {
        $setter = 'set' . ucfirst($property);

        $value = [];
        foreach ($list as $objectMetadataData) {
            [$fieldName, $object, $columns, $data] = $objectMetadataData;
            $objectMetadata = new DataObject\Data\ObjectMetadata($fieldName, $columns, $object);
            $objectMetadata->setData($data);
            $value[] = $objectMetadata;
